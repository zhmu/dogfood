...
namespace ext2
{
    namespace
    {
        ...
        void ReadBlocks(bio::Device dev, bio::BlockNumber blockNr, unsigned int count, Buffer* dest)
        {
        ...
        template<typename Buffer>
        void WriteBlocks(
            bio::Device dev, bio::BlockNumber blockNr, unsigned int count, const Buffer* source)
        {
            for (unsigned int n = 0; n < count; ++n) {
                auto& buf = bio::bread(dev, blockNr + n);
                memcpy(
        ...
        void ReadBlockGroup(bio::Device dev, const int bgNumber, BlockGroup& blockGroup)
        {
        ...
        void WriteBlockGroup(bio::Device dev, const int bgNumber, BlockGroup& blockGroup)
        {
            auto& buf = bio::bread(dev, CalculateBlockGroupBioBlockNumber(bgNumber));
            memcpy(
        ...
        void UpdateSuperblock(bio::Device dev)
        {
    ...
    void ReadInode(bio::Device dev, fs::InodeNumber inum, Inode& inode)
    {
    ...
    void UpdateInodeBlockGroup(const bio::Device dev, fs::InodeNumber inum, Function f)
    {
        inum--; // inode 0 does not exist and is not stored
    ...
    bool AllocateFromBitmap(
        const bio::Device dev, const uint32_t initialBlockGroup, uint32_t& inum, Strategy)
    {
    ...
    template<typename Strategy>
    bool
    FreeFromBitmap(const bio::Device dev, const uint32_t bgroup, const uint32_t itemIndex, Strategy)
    {
        constexpr auto bitsPerBlock = bio::BlockSize * 8;
        BlockGroup blockGroup;
        ReadBlockGroup(dev, bgroup, blockGroup);
    ...
    bool FreeDataBlock(const bio::Device dev, uint32_t blockNr)
    {
    ...
    void TraverseBlockPointers(const bio::Device dev, const uint32_t blockNr, Func f)
    {
    ...
    bool ReadDirectory(fs::Inode& dirInode, fs::Offset& offset, fs::DEntry& dentry)
    {
        union {
            DirectoryEntry de;
    ...
    bool WriteDirectoryEntry(
        fs::Inode& dirInode, fs::Offset offset, fs::InodeNumber inum, uint16_t newEntryRecordLength,
